                        ...
                        {activeTab === 'profile' && (
                            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    {[
                                        { label: 'Weight Signal', val: `${(container.Measured_Weight || 0).toLocaleString()} kg`, icon: <Weight size={14} /> },
                                        { label: 'Value Density', val: `$${(container.Declared_Value || 0).toLocaleString()}`, icon: <DollarSign size={14} /> },
                                        { label: 'Dwell History', val: `${container.Dwell_Time_Hours} Hours`, icon: <Clock size={14} /> },
                                        { label: 'Origin Audit', val: container.Origin_Country, icon: <MapPin size={14} /> }
                                    ].map((stat, i) => (
                        ...
                        {activeTab === 'analysis' && (
                            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="h-full flex flex-col space-y-10">
                                ...
                                <div className="space-y-4">
                                    ...
                                    {[
                                        { title: 'Tactical Inspection', desc: 'Execute immediate physical scan for mass discrepancies.', icon: <Scan size={22} />, color: 'text-warning', bg: 'bg-warning/10' },
                                        { title: 'Authority Audit', desc: 'Sanctions check and manifest veracity verification.', icon: <Fingerprint size={22} />, color: 'text-primary', bg: 'bg-primary/10' }
                                    ].map((item, i) => (
                                        <div key={i} className="flex items-center gap-5 p-5 bg-background/40 rounded-3xl border border-surfaceHighlight/40 hover:border-primary/30 transition-all group cursor-pointer hover:shadow-lg">
                                            <div className={`p-4 rounded-2xl ${item.bg} ${item.color} group-hover:scale-110 transition-transform shadow-sm`}>{item.icon}</div>
                                            <div>
