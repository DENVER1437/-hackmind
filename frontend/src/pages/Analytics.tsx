            ...
            <div className="dashboard-card border-surfaceHighlight/50 bg-surface/60 backdrop-blur-3xl p-8 relative overflow-hidden group">
                ...
                <div className="flex flex-col lg:flex-row gap-8 items-center relative z-10">
                    ...
                    <div className="text-lg md:text-xl font-bold text-text-main leading-relaxed font-mono flex-1">
                        <Typewriter text={`TACTICAL SYNTHESIS: ${criticalRatio}% of analyzed batch shows extreme risk probability. Cluster analysis in ${geoStats[0]?.name} suggests a weight-deviation trend. Correlation matrix confirms high-value cargo at $${(totalValue / predictions.length / 1000).toFixed(1)}k avg is disproportionately flagged. Recommendation: Execute Phase Alpha Inspection Plan.`} />
                    </div>
                </div>
            </div>
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <ScatterChart margin={{ top: 20, right: 20, bottom: 40, left: 20 }}>
                                ...
                                <XAxis
                                    ...
                                    label={{ value: 'Declared Valuation', position: 'bottom', offset: 20, fill: chartTextColor, fontSize: 10, fontWeight: 'bold' }}
                                />
                                <YAxis
                                    type="number"
                                    dataKey="score"
                                    stroke={chartTextColor}
                                    tick={{ fontSize: 10 }}
                                    label={{ value: 'Threat Index', angle: -90, position: 'insideLeft', fill: chartTextColor, fontSize: 10, fontWeight: 'bold' }}
                                />
