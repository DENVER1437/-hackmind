...
export function PredictionsTable() {
    ...
    const filteredData = useMemo(() => {
        return predictions.filter(item => {
            const matchesFilter = filter === 'All' || item.Risk_Level === (filter === 'Moderate' ? 'Medium' : filter);
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8 relative">
                ...
                <div className="flex flex-wrap gap-4 w-full md:w-auto z-10">
                    ...
                    <div className="relative group">
                        <label className="absolute -top-5 left-1 text-[10px] font-black text-text-muted uppercase tracking-[0.2em] opacity-60">Intensity Filter</label>
                        <select
                            className="appearance-none bg-surface border border-surfaceHighlight/50 text-text-main py-3 pl-4 pr-12 rounded-2xl focus:outline-none focus:border-primary/50 text-sm font-black uppercase tracking-widest cursor-pointer hover:bg-surfaceHighlight/20 transition-all shadow-inner"
                            value={filter}
                        ...
                        <ChevronDown size={14} className="absolute right-4 top-1/2 transform -translate-y-1/2 text-primary group-hover:scale-125 transition-transform pointer-events-none" />
                    </div>
            ...
            <div className="dashboard-card p-0 overflow-hidden border-surfaceHighlight/30 shadow-2xl relative group">
                <div className="absolute inset-0 bg-gradient-to-br from-primary/5 to-transparent pointer-events-none opacity-40 -z-10" />
                                ...
                                {paginatedData.map((item, idx) => (
                                    <motion.tr
                                        ...
                                        className={`group/row cursor-pointer transition-all duration-300 border-l-[4px] shadow-sm active:scale-[0.99] ${item.Risk_Level === 'Critical' ? 'border-l-critical bg-critical/[0.02] hover:bg-critical/[0.05]' :
                                                item.Risk_Level === 'Medium' ? 'border-l-warning bg-warning/[0.02] hover:bg-warning/[0.05]' :
                                    ...
                                    >
                                        ...
                                        <td className="p-6">
                                            <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${item.Risk_Level === 'Critical' ? 'bg-critical/10 text-critical border-critical/30 shadow-[0_0_15px_rgba(239,68,68,0.1)]' :
                                                    item.Risk_Level === 'Medium' ? 'bg-warning/10 text-warning border-warning/30' :
                                                        'bg-success/10 text-success border-success/30'
                                                }`}>
                                                <div className={`w-2 h-2 rounded-full active-glow ${item.Risk_Level === 'Critical' ? 'bg-critical animate-pulse' : item.Risk_Level === 'Medium' ? 'bg-warning' : 'bg-success'
                                                    }`} />
                                                {item.Risk_Level === 'Medium' ? 'Moderate' : item.Risk_Level}
                                            </div>
                                        </td>
                                        <td className="p-6">
                                            {item.Anomaly_Flag === 'Yes' ? (
                ...
                <div className="p-8 border-t border-surfaceHighlight/20 flex flex-col sm:flex-row items-center justify-between gap-8 bg-surface/30 backdrop-blur-md relative z-10">
            ...
            <style>{`
                @media print {
                    .dashboard-card { border: none !important; box-shadow: none !important; background: white !important; }
                    button, input, select, label { display: none !important; }
                    table { color: black !important; }
                    th { color: #333 !important; border-bottom: 2px solid #333 !important; padding: 10px !important; }
                    td { border-bottom: 1px solid #eee !important; padding: 10px !important; }
                    .glow-critical, .bg-primary { filter: none !important; box-shadow: none !important; }
                }
            `}</style>
        </div>
    );
